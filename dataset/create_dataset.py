...
def load_emb(w2i, path_to_embedding, embedding_size=300, embedding_vocab=2196017, init_emb=None):  # w2i len: 15063
    if init_emb is None:
    ...
    return torch.tensor(emb_mat).float()  # (15063, 300)
...
class UR_FUNNY:
    def __init__(self, config):
        ...
        try:
            ...
            for key in humor_label_sdk.keys():
                ...
                label = np.array([np.nan_to_num(label)])
                _visual = np.nan_to_num(_visual)
                _acoustic = np.nan_to_num(_acoustic)
    ...
    def get_data(self, mode):
        if mode == "train":
            ...
            #     sent = " ".join(sample[0][3])                
            #     encoded_bert_sent = tokenizer.encode_plus(sent, add_special_tokens = False, max_length = 50, padding = 'max_length', truncation = True)
            #     if len(encoded_bert_sent['input_ids'])==0 or len(encoded_bert_sent['input_ids'])!=50:
            #         print(sent)
            #     res.append(encoded_bert_sent)
