...
def load_checkpoint(model, checkpoint_path, use_ema=True, strict=True, remap=False):
    ...
    # Added mismatched keys
    mismatched_keys = ['head.weight','head.bias']
    new_state_dict = { k:v for k, v in state_dict.items() if k in model.state_dict() and k not in mismatched_keys}    
