...
class pointwhCoder(pointCoder):
    ...
    def meshgrid(self, boxes):
        ...
        xs = torch.nn.functional.interpolate(xs, size=self.patch_pixel, mode='linear', align_corners=False)
        ys = torch.nn.functional.interpolate(ys, size=self.patch_pixel, mode='linear', align_corners=False)
